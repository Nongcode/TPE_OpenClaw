...
const DEFAULTS = {
  ...
  project_url: "https://labs.google/fx/vi/tools/flow/project/64c7c243-037d-4136-a867-4ba4d834605b",
  target_gemini_url: "https://labs.google/fx/vi/tools/flow/project/64c7c243-037d-4136-a867-4ba4d834605b",
  image_prompt: "",
  image_paths: [],
  reference_image: "",
  logo_paths: [],
...
function parseArgs(argv) {
  ...
  if (args.findIndex((a) => a === "--input_file" || a === "--input-file") !== -1) {
    ...
    if (args[idx + 1]) {
      try {
        ...
        if (parsed.target_gemini_url && !parsed.project_url) params.project_url = parsed.target_gemini_url;
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (token === "--prompt" || token === "--image_prompt") {
      ...
      index += 1;
    } else if (token === "--target_gemini_url") {
      params.target_gemini_url = next;
      params.project_url = next;
      index += 1;
  ...
  if (params.target_gemini_url && !params.project_url) params.project_url = params.target_gemini_url;
  if (params.project_url && !params.target_gemini_url) params.target_gemini_url = params.project_url;
...
async function waitForNewVideoSource(page, beforeVideoSources, timeoutMs, logs) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (freshSources.length > 0) {
      logs.push(`[step6] Detected new image source: ${freshSources[0]}`);
  ...
  throw new Error("Timeout waiting for a new image source generated after submit");
}

async function waitForVideoResource(page, timeoutMs, logs, beforeVideoSources) {
  logs.push(`[step6] Waiting up to ${timeoutMs / 1000}s for newly generated image...`);
...
function qualityRank(text) {
  ...
  if (normalized.includes("2k") || normalized.includes("2048p")) return 2048;
...
function normalizeResolutionLabel(text) {
  ...
  if (normalized.includes("2k") || normalized.includes("2048p")) return "2k";
...
async function saveDownloadWithMediaExtension(download, outputDir, logs, selectedResolution) {
  ...
  const normalizedSelected = normalizeResolutionLabel(selectedResolution) || "2k";
  ...
  const hasKnownMediaExt = /\.(png|jpg|jpeg|webp|gif)$/i.test(currentExt);
  ...
  if (!fileName || !hasKnownMediaExt || looksLikeUuid) {
    ...
    if (!stem || looksLikeUuid) {
      stem = `flow-image-${normalizedSelected}-${nowStamp()}`;
    }
...
function buildMediaOutputPath(outputDir, selectedResolution) {
  ...
  const fileName = `flow-image-${normalizedSelected}-${nowStamp()}${ext}`;
...
async function findLeftmostVideoCard(page, logs) {
  ...
  if (cardCandidates.length === 0) {
    logs.push("[step7] Could not detect any visible image card candidate");
    return null;
  }
...
async function tryDownloadNewestVideoBySource(
  ...
  logs.push("[step7] Direct source fallback: downloading newest image by source URL");
...
async function downloadVideoByKnownSource(
  ...
  if (!response || !response.ok()) {
    throw new Error(`Khong the tai anh tu source moi: ${sourceUrl}`);
  }

  const body = await response.body();
  if (!body || body.length < 1024) {
    throw new Error("Downloaded image payload is empty or too small");
  }

  const destination = buildMediaOutputPath(outputDir, preferredResolution);
  await writeFile(destination, body);
  logs.push(`[step7] Downloaded generated image by known source -> ${destination}`);
  return destination;
}
...
async function pickResolutionAndDownload(page, outputDir, logs, preferredResolution) {
  const qualityCandidates = page
    ...
    .filter({ hasText: /4k|2160p|2k|2048p|1440p|1080p|720p|480p|360p|270p/i });
...
async function openLatestCardPreview(page, target, logs) {
  ...
  logs.push("[step7] Opening preview of the leftmost (newest) image card...");
...
async function runPostGenerationQc({ imagePath, referenceImage, logs }) {
  if (!imagePath || !String(imagePath).trim()) {
    return {
      pass: false,
      score: 0,
      reason: "Image QC fail: khong co duong dan anh sau generate",
    };
  }
  ...
  if (!existsSync(resolvedImagePath)) {
    return {
      pass: false,
      score: 0,
      reason: `Image QC fail: anh khong ton tai ${resolvedImagePath}`,
    };
  }
  if (referenceImage && !existsSync(resolvedReferenceImage)) {
    return {
      pass: false,
      score: 0,
      reason: `Image QC fail: reference image khong ton tai ${resolvedReferenceImage}`,
    };
  }

  const imageBuffer = readFileSync(resolvedImagePath);
  if (!imageBuffer || imageBuffer.length < 1024) {
    return { pass: false, score: 0, reason: "Image QC fail: file anh rong hoac qua nho" };
  }
  ...
  const qcResult = {
    ...
    score: 0.35,
    reason:
      "QC da xac nhan file anh ton tai, khong rong va Flow da chay qua luong upload/prompt/download.",
  };
...
(async function main() {
  ...
  const flowUrl = target_gemini_url || project_url;
  const imagePrompt = String(prompt || "").trim();
  const normalizedAssetPaths = parseList(image_paths).map((filePath) => path.resolve(filePath));
  const uploadReferenceImage = normalizedAssetPaths[0] || "";
  const outDir = path.resolve(output_dir || path.join(process.cwd(), "artifacts", "images"));
  await mkdir(outDir, { recursive: true });

  logs.push(`[input] target_gemini_url=${flowUrl}`);
  logs.push(`[input] browser=${browser_path} ${profile_name}`);
  logs.push(`[input] target_dir=${outDir}`);
  logs.push(`[input] image_paths=${normalizedAssetPaths.length}`);

  if (!imagePrompt) {
    logs.push("[fail] Missing image_prompt");
    printResult(
      buildResult({
        success: false,
  ...
  try {
    await ensureReadableFiles(normalizedAssetPaths);
    if (normalizedAssetPaths.length > 0) {
    ...
    printResult(
      buildResult({
        ...
        logs,
        error: { code: "IMAGE_FILE_ERROR", details: err.message, failed_step: 3 },
      }),
    );
    process.exit(1);
  }
  ...
  if (parsed.dry_run) {
    ...
    printResult(
      buildResult({
        ...
        data: {
          target_gemini_url: flowUrl,
          image_prompt: imagePrompt,
          image_paths: normalizedAssetPaths,
        },
        logs,
      }),
    );
  ...
  try {
    ...
    logs.push(`[step2] Navigating to ${flowUrl}`);
    ...
    const screenshotBefore = path.join(outDir, `flow-image-before-${nowStamp()}.png`);
    ...
    if (!(await waitForPromptValue(page, imagePrompt, logs, 5000))) {
    ...
    await submitPrompt(page, logs, imagePrompt);

    const waitResult = await waitForVideoResource(page, timeout_ms, logs, beforeVideoSources);
    const newImageSource = waitResult.source;

    let downloadedImagePath = null;
    try {
      downloadedImagePath = await downloadVideoByKnownSource(
        context,
        newImageSource,
        outDir,
        logs,
      ...
      downloadedImagePath = await tryDownloadLatestVideoWithFallbacks(
        context,
        page,
        outDir,
        logs,
    ...
    const screenshotAfter = path.join(outDir, `flow-image-after-${nowStamp()}.png`);
    ...
    let relImagePath = null;
    if (downloadedImagePath) {
      relImagePath = path.relative(process.cwd(), downloadedImagePath).replace(/\\/g, "/");
      artifacts.push({ type: "generated_image", path: relImagePath });
    }

    logs.push("[step8] Flow finished successfully");
    const chatReply = downloadedImagePath
      ? buildChatImageReplyPayload({
          imagePath: downloadedImagePath,
          data: {
            target_gemini_url: flowUrl,
            image_prompt: imagePrompt,
            downloaded_image_path: relImagePath || downloadedImagePath,
          },
          artifacts,
        })
      : null;
      ...
      buildResult({
        ...
        message:
          ...
          (downloadedImagePath
            ...
            : "Image generated but could not be downloaded."),
        data: {
          target_gemini_url: flowUrl,
          image_prompt: imagePrompt,
          downloaded_image_path: relImagePath || downloadedImagePath,
          used_image_paths: normalizedAssetPaths,
          reference_image_sha256: uploadReferenceImage
            ? computeFileSha256(uploadReferenceImage)
            : null,
          image_qc_status: "PASS",
          image_qc_reason: qcResult.reason,
          generation_status_message: "Flow image generation completed",
          ...(chatReply?.data || {}),
        },
    ...
    printResult(
      buildResult({
        ...
        data: { target_gemini_url: flowUrl, image_prompt: imagePrompt },
        artifacts,
        logs,
