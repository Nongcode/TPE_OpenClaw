...
function resolveAutomationSessionKey(agentId, workflowId, stepId, candidateSessionKey = "") {
  const candidate = String(candidateSessionKey || "").trim();
  if (
    candidate &&
    (candidate.includes(":automation:") ||
      (workflowId && candidate.includes(String(workflowId))))
  ) {
    return candidate;
  }
  return buildWorkflowScopedSessionKey(agentId, workflowId, stepId);
}

function buildWorkflowTaskId(workflowId, stepId) {
  const safeWorkflowId = String(workflowId || "").trim() || `wf_runtime_${Date.now()}`;
  const safeStepId = String(stepId || "").trim() || "step";
  return `task_${safeWorkflowId}_${safeStepId}`;
}
...
async function resolveRootWorkflowBinding(context) {
  ...
  try {
    const resolved = await beClient.resolveAutomationRootConversation({
      ...
      managerInstanceId: explicitManagerInstanceId || undefined,
    });
...
async function resolveWorkflowSessionContext(params) {
  const taskId = params.taskId || buildWorkflowTaskId(params.workflowId, params.stepId);
  let actualSessionKey = resolveAutomationSessionKey(
    params.agentId,
    params.workflowId,
    params.stepId,
    params.sessionKey,
...
async function runAgentStepDetailed(params) {
  const { sessionKey: actualSessionKey, subConv, taskId } = await resolveWorkflowSessionContext(params);
...
async function tryRecoverAsyncStageState(params) {
  ...
  let workerSessionKey = resolveAutomationSessionKey(
    spec.agentId,
    state.workflow_id,
    spec.stepId,
